    ...
    async with async_playwright() as p:
        ...
        if calorie_display:
            ...
            if "150" in text or "2400" in text:
                print("✅ Calorie summary shows correct data\n")
        ...
        if meal_form:
            ...
            if meal_input and date_input:
                # Get the date value
                js = (
                    "var f = document.querySelector('form[action=\"/nutrition/add_meal/\"]');"
                    f"f.querySelector('input[name=\"meal_name\"]').value = 'Lunch';"
                    f"f.querySelector('input[name=\"date\"]').value = '{today}';"
                )
                await page.evaluate(js)
                print("✅ Add Meal form fields populated\n")
